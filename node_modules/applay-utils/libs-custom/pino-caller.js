...
function traceCaller(pinoInstance, options = { relativeTo: null, stackAdjustment: 0, position: null, prefix: null }) {
  ...
  function asJson(...args) {
    ...
    if (options && typeof options.relativeTo === 'string') {
      ...
      let position = args[0].caller.replace(')', '').split(':').slice(1)
      if (options.position) {
        position = options.position;
      }

      args[0].caller = ` ${options.prefix || 'anonymous'} => (${position[0]}:${position[1]}) `
